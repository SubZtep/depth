...
.flex
  .flex-1
    ...
    br
    input.w-full(type="range" min="1" max="10" v-model.number="state.modelValue[0]" v-if="props.hover !== false")
  .flex-1
    | {{props.labels?.[1] ?? "Y"}}:
    input.w-8.ml-1.filter(type="text" v-model.number="state.modelValue[1]" :class="{ 'invert': props.hover === false}")
    br
    input.w-full(type="range" min="1" max="10" v-model.number="state.modelValue[1]" v-if="props.hover !== false")
</template>
